...
export function ParticleBackground() {
  ...
  useEffect(() => {
    ...
    // Interaction tuning
    const INTERACT_RADIUS = 110; // small area around cursor
    const INTERACT_RADIUS_SQ = INTERACT_RADIUS * INTERACT_RADIUS;
    const FORCE_STRENGTH = 0.12; // gentle
    const MAX_SPEED = 0.9;
    const FRICTION = 0.94; // ease back to slow drift
    ...
    const tick = () => {
      ...
      for (const p of particles) {
        ...
        // parallax
        const ox = -px * 9 * p.parallax;
        const oy = -py * 9 * p.parallax;
        const cx = p.x + ox;
        const cy = p.y + oy;
        ...
        grad.addColorStop(0.7, `rgba(${baseColor}, ${p.alpha * 0.4})`);
        ...
        // ultra-thin bubble rim
        ctx.beginPath();
        ctx.arc(cx, cy, p.r, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(${baseColor}, ${p.alpha * 0.7})`;
        ctx.lineWidth = 0.4;
        ctx.stroke();
      }
